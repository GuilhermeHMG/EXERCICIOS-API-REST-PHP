...
$json = json_decode($contents, true);                               //º - Estamos codificando em Json o que estiver no nosso banquinho de dados, e informando 'true' para ele trazer como ARRAY
...
if($method === 'GET')                                               //º - SE o método de requisição for GET, vamos retornar o nosso Json
{
    if($json[$path[0]])                                             //º - Se eu achei DENTRO do'$json (db.json)' na posição '[$path[0]] o caminho 'series' ("series":[],"genres":[])
    {
        ...
        else
        {
            ...
            if($encontrado >= 0)                                    //º - Então, se o $encontrado receber 0, 1, 2 ...
             {
                echo json_encode($json[$path[0]][$encontrado]);     //º - ...vai mostrar na tela os dados da 'series(objeto 'series' dentro do json - $json[$path[0]])', que está na posição '$encontrado' do json. 
    ...
    {
        echo 'Não há séries para listar.';                          //º - Retorno uma string vazia.
    }   
}
...
{
    $jsonBody = ($body);                                            //º - Vou pegar o '$body' que É UM JSON DECODIFICADO (linha 14), e vamos processar ele para por um
    ...
    echo json_encode($jsonBody, true);                              //º - Depois disso, vou escrever (echo) em Json (json_encode) o que tem no '$jsonBody' para mostrar ao solicitante
